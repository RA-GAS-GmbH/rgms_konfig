...
const CSV_RREG: &str = "src/sensors/sensor_mb_ne4_legacy-rregs.csv";
const CSV_RWREG: &str = "src/sensors/sensor_mb_ne4_legacy-rwregs.csv";
...
impl SensorMbNe4Legacy {
    ...
    /// assert!(sensor.rregs.len() == 16);
    /// assert!(sensor.rwregs.len() == 44);
    /// ```
    pub fn new_from_csv() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        Ok(SensorMbNe4Legacy { rregs: rregs?, rwregs: rwregs? })
    }
}
